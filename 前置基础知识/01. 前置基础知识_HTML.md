...
1. 文件准备  
   - 沿用文件夹，新建 `test5.html` 与 `reg-style.css`（可选美化）  
...
5. 验证步骤  
   ...
   - 改单选/下拉值为随意字符串（DevTools 改 `value`）→ 提交后理解“后端必须重验”
...
| `<article>` | 独立的"文章内容块"，可单独存在（如博客文章、新闻、课程卡片） | 博客列表、新闻列表、课程卡片                         | ```html <article> <h2>XSS 攻击原理与防御</h2> <p>XSS 攻击是指通过注入恶意脚本...</p> <footer>作者：安全讲师</footer> </article> ``` |
...
| `<footer>`  | 页面或区域的"底部"，通常包含版权信息、联系方式、备案号等     | 网页底部、文章底部、板块底部                         | ```html <footer> <p>© 2024 网络安全学习平台 版权所有</p> </footer> ``` |
...
2. 升级点（共 4 处，其余保持不动）  
   ...
   ② **新增**一行“手机”输入框：  
      ...
      - 加 `min="1900-01-01"` `max="2024-12-31"`、`required`  
        ④ 按钮区保留原“提交/重置”，**再加一个**`type="reset"` 按钮，文字 `"一键清空"`（实现双重置）
...
<body>
  ...
  <form action="" method="POST" enctype="multipart/form-data">
    ...
    <p>
      <label>邮　箱：</label>
      <input type="email" name="user" placeholder="name@example.com" required>
    </p>

    <!-- ② 新增手机 -->
    <p>
      <label>手　机：</label>
      <input type="tel" name="phone" pattern="1[3-9]\d{9}" placeholder="11位手机号" required>
    </p>
    ...
    <p>
      <label>生　日：</label>
      <input type="date" name="birth" min="1900-01-01" max="2024-12-31" required>
    </p>
